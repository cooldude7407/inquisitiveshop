...
module.exports.run = async (bot, message, args) => {
    if (message.author.id != 688440007153025075) return message.reply("You can't use this command! It's Staff only!");
    ...
    try {
        await rM.send(`You have been owner bypass given the role: ${roleg.name}`)
    } catch (e) {
        message.channel.send(`<@${rM.id}> has been owner bypass given the role: ${roleg.name}`)
    }
};
module.exports.help = {
    name: "sbaddrole"
